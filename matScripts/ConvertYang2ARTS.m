...
[nrows1,ncols1,nw1,nsizes] = DataSpecs('Data_0.2_15.25/');
[nrows2,ncols2,nw2,nsizes] = DataSpecs('Data_16.4_99.0/');

%ihabits = {'5_plates/','8_columns/','10_plates/','droxtal/',...
%    'HBR/','hollow_column/','plate/','SBR/','single_column/'};

%ihabits = {'plate/'};
ihabits = {'SBR/','single_column/'};
%iroughs = {'Rough050'};
...
for hindex = 1:length(ihabits)
    ...
    for rindex = 1:length(iroughs)
        ...
        %- Define input/output paths for wavelegth range 1
        paths1 = SwitchUser('Data_0.2_15.25/', ihabit, iroughness);
        ...
        paths2 = SwitchUser('Data_16.4_99.0/', ihabit, iroughness);
	...
	disp('High done')
	   ...
	   %------------------
           ...
           S1(isize).ext_mat_data=[S1(isize).ext_mat_data; S2(isize).ext_mat_data]; % without ; cannot combine non equal dims
